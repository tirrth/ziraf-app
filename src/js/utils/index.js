...
const utils = {
	api: {
		get: (path, data) => {
			return fetch(
				BaseAjaxConfig.host + path + ('?' + qs.stringify(data)),
				{
					method: 'GET',
	...
	getZirafSecretCode: () => {
		return fetch(
			BaseAjaxConfig.host + "/api/v1/secretcodes",
			{
				method: 'GET',
				headers: BaseAjaxConfig.headers
			}
	...
	getAdPopupBanners: () => {
		return fetch(
			BaseAjaxConfig.host + "/api/v1/ads/active",
			{
				method: 'GET',
				headers: BaseAjaxConfig.headers
			}
	...
	restaurantSendAnalytics: data => {
		return fetch('https://www.zirafapp.com/analytics/restaurant_analytics.php', {
			method: 'POST',
			headers: BaseAjaxConfig.headers,
			body: JSON.stringify(data)
		});
	...
	getSortedReviews: (data, sortBy) => {
		...
		return data.sort((a, b) => {
			switch (sortKey) {
				...
				case 'name':
					if (sortBy[sortKey] === 'ascending') {
						if (a.zirafer.displayName < b.zirafer.displayName)
							return -1;
						if (a.zirafer.displayName > b.zirafer.displayName)
							return 1;
						return 0;
	...
	getAllRestaurantsSimple: () => {
		return fetch(
			BaseAjaxConfig.host + "/api/v1/restaurants/simple",
			{
				method: 'GET',
				headers: BaseAjaxConfig.headers
			}
