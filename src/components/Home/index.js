...
const getItemLayout = (data, index) => (
	{length: 205, offset: 205 * index, index}
);
...
class HomeScreen extends Component {
			...
			getAdPopupBanners().then(resp => {
				if (resp.success) {
					...
					const popupBannerList = [
						...new Set(
							unfilteredpopuplist.map(obj => JSON.stringify(obj))
						)
	...
	fetchModifiedFiltersAndSearchText(filters) {
		const { searchText, previousSearchTermId, previousSearchTermType, autocompleteList } = this.state;
		
		let modifiedFilters = filters;
		let confirmedSearchText = '';

		if(autocompleteList) {
			let tempData = autocompleteList.filter(item => 
				...
				if (previousSearchTermId != '' || previousSearchTermType != '') {
					if (previousSearchTermType === 'Cuisines') {
						if (modifiedFilters['cuisines']) {
							const idx = modifiedFilters['cuisines'].indexOf(previousSearchTermId);
							if (idx != -1) {
						...
						if (modifiedFilters['moments']) {
							const idx = modifiedFilters['moments'].indexOf(previousSearchTermId);
							if (idx != -1) {
		...
		if (modifiedFilters.cuisines == null && 
	...
	fetchData() {
		...
		return fetchRestaurantList(modifiedFilters, confirmedSearchText, location, sorting);
	}
	...
	componentDidUpdate(prevProps) {
		...
		if (
			this.props.appState && this.props.appState.FETCH_FILTERED_DATA
			&& this.props.appState.FETCH_FILTERED_DATA != prevProps.appState.FETCH_FILTERED_DATA
			) {
	...
	static getDerivedStateFromProps(nextProps, prevState) {
		if (nextProps.autoComplete && nextProps.autoComplete !== prevState.autoComplete) {
	...
	render() {
		const { userDetail, restaurantList, navigation, appState, autoComplete } = this.props;
		const {
			modalX,
			sorting,
			searchText,
		...
		return (
			<View style={styles.container}>
				...
				{showAutocompleteOptions && (
					<View
						style={(appState.SHOW_FILTER_VIEW || appState.SHOW_MY_VIEW) ? {
							position: 'absolute',
							width: '100%',
							left: 25,
							right: 0,
							...
							appState.POPUPS_AVAILABLE ? (
								<View style={styles.popupButton}>
									<View
										...
										<TouchableOpacity
											onPress={() => {
												this.setPopupBannerDetailsVisible(
													true
												);
											}}
											style={{ flex: 9 }}>
				...
				<Modal
					...
					<View style={{ flex: 1 }}>
						<OnboardingModal
							setModalVisible={this.setOnboardingModalVisible.bind(
								this
							)}
							navigation={navigation}
						/>
				...
				<Modal
					...
					<View style={{ flex: 1 }}>
						<PopupBanner
							setModalVisible={this.setPopupBannerDetailsVisible.bind(
								this
							)}
							data={popups}
							adPopupClickedEvent={this.adPopupClickedEvent.bind(
								this
							)}
						/>
					</View>
