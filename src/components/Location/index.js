...
class Location extends Component {
  ...
  updateMap() {
    const {appState, navigation, route} = this.props;
    // const locations = navigation.getParam('location');
    const locations = route?.params?.location;
    let location = {};
    location['latitude'] = appState.CURRENT_LOCATION.coords.latitude;
    location['longitude'] = appState.CURRENT_LOCATION.coords.longitude;
  ...
  setSearch() {
    ...
    if (locations?.lon && locations?.lat) {
      location['latitude'] = parseFloat(locations.lat.toFixed(4));
      location['longitude'] = parseFloat(locations.lon.toFixed(4));
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const prevPropsLocation = prevProps?.route?.params?.location;
    // if (
    ...
    if (prevProps.navigation && prevPropsLocation !== location) {
      // this.updateMap();
    }

    if (prevProps.appState && prevProps.appState.FILTERS !== appState.FILTERS) {
      // this.updateMap();
    }
    // this.updateMap();

    if (navigation.navigation !== prevProps.navigation) {
      this.setSearch();
    }
  }
          ...
          {restaurantLocations.length
            ? restaurantLocations
                .filter(rest => {
                  ...
                  /* navigation.getParam('restaurantId') */
                  ...
                  <Marker
                    ...
                    <Callout
                      ...
                      <View
                        ...
                        <Text style={[cs.textBold, cs.textGray, cs.font18]}>
                          {reg.title}
                        </Text>
                        <Text
                          style={[cs.textGray, cs.font12, {marginBottom: 5}]}>
                          {reg.description}
                        </Text>
                        <Text style={[cs.textOrange, cs.font12, cs.textCenter]}>
        ...
        <View style={[styles.googlePlacesSearchContainer]}>
          ...
          {appState.FILTERS ? (
            <TouchableOpacity
              ...
              <Text style={[cs.textBold, cs.textOrange, cs.font12]}>
                Clear Filter
              </Text>
            </TouchableOpacity>
          ) : null}
            ...
            restaurantList.data.length < restaurantList.count ? (
              <View style={[styles.bottomButtonContainer]}>
                <Text
                  ...
                  Showing {restaurantList.data.length} / {restaurantList.count}{' '}
                  Restaurants
                </Text>
                <TouchableOpacity
                  disabled={loading}
                  ...
                  <Text style={[cs.textBold, cs.font12]}>Load more</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <View style={[styles.bottomButtonContainer]}>
                <Text
                  ...
                  Showing {restaurantList.data.length} Restaurants
                </Text>
              </View>
            )
          ) : null}
              ...
              {route?.params?.direction ? (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    const direction = route?.params?.direction;
                    this.handleOpenURL(
                      /* navigation.getParam('direction') */ direction,
                    );
                  }}>
