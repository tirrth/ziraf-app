    ...
    return (
      <View style={{flexDirection: 'row', flexWrap: 'wrap'}}>
        {data.map((item, idx) => {
          ...
          return (
            <TouchableOpacity
              onPress={() => this.props.toggleSelect(keyName, item.id)}
              key={idx}
              style={{
                width: '33%',
                flexDirection: 'row',
  ...
  render() {
    ...
    return (
      <MultiSlider
        ...
        min={data.min}
        max={data.max}
        onValuesChange={values => sliderValuesChange(filterName, values)}
        values={
          ...
          <View
            ...
            <View style={{width: '50%', alignItems: 'center'}}>
              <RangeSlider
                ...
                filterName={filter}
                data={data[filter]}
                sliderValuesChange={this.sliderValuesChange.bind(this)}
              />
            </View>
            <Text
              allowFontScaling={false}
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
          ...
          <TouchableOpacity onPress={() => this.setMyView()}>
            <Text
              style={[
                cs.font12,
                cs.textBold,
                currentView === 'my-view' ? cs.textOrange : cs.textWhite,
              ]}>
              {currentView === 'my-view' ? 'Your View' : 'Create Your View'}
            </Text>
          </TouchableOpacity>
        ...
        <ScrollView>
          <View style={[cs.paddingTB15, {paddingLeft: 20, paddingRight: 20}]}>
            {appConfig && appConfig.data && appConfig.data.filters
              ? Object.keys(appConfig.data.filters).map((filter, idx) => (
                  <Collapsable
                    onToggle={this.handleCollapsableToggle.bind(this, filter)}
                    key={'acc_' + idx}
                    item={{
                    ...
                    isCollapsed={
                      !(
                        selected &&
                        selected.length > 0 &&
                        selected.indexOf(filter) > -1
                      )
                    }
                    isCollapseable={
                      filter !== 'delivery' && filter !== 'reservation'
                    }
                    iconActive={
                      filter === 'delivery'
          ...
          <View style={{alignItems: 'center'}}>
            <TouchableOpacity
              ...
              <Text style={[cs.textWhite, cs.textBold]} fontVisby={true}>
                GO
              </Text>
            </TouchableOpacity>
          </View>
