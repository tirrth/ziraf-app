...
class Zirafer extends Component {
  ...
  componentDidMount() {
    const {fetchZiraferDetailData, navigation, route} = this.props;
      ...
      <SafeAreaView style={{flex: 1, backgroundColor: '#1d1d1c'}}>
        <View style={styles.container}>
          <ScrollView>
            ...
            <View style={[styles.displayNameContainer, {padding: 15}]}>
              <Text
              ...
              <View style={[styles.userContainer]}>
                ...
                zirafer.data.speciality.length > 0 ? (
                  <Text allowFontScaling={false} style={styles.info}>
                    Speciality:{' '}
                    {zirafer.data && zirafer.data.speciality.join(', ')}
                  </Text>
                ) : null}
                {zirafer.data &&
                ...
                zirafer.data.favouriteRestaurant.length > 0 ? (
                  <Text allowFontScaling={false} style={styles.info}>
                    ...
                    {zirafer.data && zirafer.data.favouriteRestaurant}
                  </Text>
                ) : null}
              ...
              zirafer.data.reviews.length > 0 ? (
                zirafer.data.reviews.map((review, idx) => (
                  <ReviewItem
                    data={review}
                    key={`review-${idx}`}
                    onPress={this.handleReviewImagePress.bind(this)}
                    onRating={ratingBreakdown =>
                      this.setState({
                        alertSize: 'small',
                        alertSuccess: true,
                        alertTitle: review.restaurantName,
                        alertDetail: '',
                        alertChildren: (
