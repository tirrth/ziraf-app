...
class Moment extends Component {
	...
	fetchData() {
		const { appState, fetchMomentRestaurantsList, navigation } = this.props;
		const { sorting } = this.state;

		let location = Object.assign({}, appState.CURRENT_LOCATION);

		return fetchMomentRestaurantsList(
	...
	render() {
		const { userDetail, navigation, momentRestaurants } = this.props;
		const { region } = this.state;
		if (!momentRestaurants) {
...
function mapStateToProps(state) {
	return {
		...
		momentRestaurants: state.momentRestaurants,
...
export default connect(
	mapStateToProps,
	{
		fetchMomentRestaurantsList: fetchMomentRestaurants
	}
)(Moment);
